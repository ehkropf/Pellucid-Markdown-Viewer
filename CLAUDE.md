...
- Release workflow: tag → GitHub release → update checksums in both Portfile and Formula → `make portindex` → commit
...
### Homebrew
- `Formula/pellucid.rb` — Homebrew formula; builds from source, in-repo
- `brew install --formula Formula/pellucid.rb` — install from local clone
- App installs to Homebrew prefix (`$(brew --prefix)/opt/pellucid/Pellucid.app`); caveats suggest symlinking to `/Applications`
- Release workflow: update `url` and `sha256` in formula after tagging
...
Formula/         — Homebrew formula (pellucid.rb)
